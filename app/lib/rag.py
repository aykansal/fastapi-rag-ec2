...
def ingest_from_url(url: str) -> int:
    ...
    # ADD-ON: for embedding_traces - Create a runnable wrapper for the embedding function
    embedding_runnable = RunnableLambda(embedder.embed_query)

    index = get_pinecone_index()

    vectors = []
    for i, chunk in enumerate(chunks):
        # Use .invoke() on the wrapper instead of .embed_query()
        vector = embedding_runnable.invoke(
            chunk.page_content, config={"callbacks": [langfuse_handler]}
        )
...
"""
def ingest_from_url2(url: str) -> int:
    # Load and parse HTML
    bs4_strainer = bs4.SoupStrainer(
        class_=("post-title", "post-header", "post-content")
    )
    ...
    for i, chunk in enumerate(chunks):
        with langfuse.start_as_current_observation(
            as_type="embedding",
            name="ingest_embed_query",
            input={"text": chunk.page_content, "chunk_index": i},
        ) as embed_span:
            vec = embedder.embed_query(chunk.page_content)
            # explicitly record the output of the embedding
            embed_span.update(output={"embedding": vec})
        vectors.append(
            {
                "id": f"doc-{i}",
                "values": vec,
...
def setup_semantic_cache():
    ...
    try:
        ...
        embedder = get_embedder()
        set_llm_cache(
            RedisSemanticCache(redis_client=redis_client, embeddings=embedder)
        )
        return True
    except Exception:
        return False
